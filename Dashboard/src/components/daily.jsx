...
export default function Daily() {
  ...
  const formatDate = (date) => {
    return format(new Date(date), 'MM-dd'); // Format as MM-dd
  };
  ...
  const [formattedData] = useMemo(() => {
    ...
    Object.values(dailyData).forEach(({ date, totalCalories, totalCarbs }) => {
      ...
      if (dateFormatted >= startDate && dateFormatted <= endDate) {
        const splitDate = formatDate(date); // Use the formatted date

        totalCaloriesLine.data.push({ x: splitDate, y: totalCalories });
        totalCarbsLine.data.push({ x: splitDate, y: totalCarbs });
      }
    });
